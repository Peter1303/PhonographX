package com.peter1303.phonograph.ui.fragments.mainactivity.library;
...
import com.peter1303.phonograph.adapter.MusicLibraryPagerAdapter;
import com.peter1303.phonograph.dialogs.CreatePlaylistDialog;
import com.peter1303.phonograph.dialogs.SleepTimerDialog;
import com.peter1303.phonograph.helper.SortOrder;
import com.peter1303.phonograph.interfaces.CabHolder;
import com.peter1303.phonograph.ui.activities.MainActivity;
import com.peter1303.phonograph.ui.activities.SearchActivity;
...
public class LibraryFragment extends AbsMainActivityFragment implements CabHolder, MainActivity.MainActivityFragmentCallbacks, ViewPager.OnPageChangeListener, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
        if (PreferenceUtil.LIBRARY_CATEGORIES.equals(key)) {
            ...
            pagerAdapter.setCategoryInfos(PreferenceUtil.getInstance(Objects.requireNonNull(getActivity())).getLibraryCategoryInfos());
            pager.setOffscreenPageLimit(pagerAdapter.getCount() - 1);
            int position = pagerAdapter.getItemPosition(current);
            if (position < 0) position = 0;
            pager.setCurrentItem(position);
            PreferenceUtil.getInstance(Objects.requireNonNull(getContext())).setLastPage(position);
    ...
    private void setUpToolbar() {
        int primaryColor = ThemeStore.primaryColor(Objects.requireNonNull(getActivity()));
    ...
    private void setUpViewPager() {
        ...
        if (PreferenceUtil.getInstance(Objects.requireNonNull(getContext())).rememberLastTab()) {
    ...
    public MaterialCab openCab(final int menuRes, final MaterialCab.Callback callback) {
        ...
        cab = new MaterialCab(getMainActivity(), R.id.cab_stub)
                ...
                .setBackgroundColor(PhonographColorUtil.shiftBackgroundColorForLightText(ThemeStore.primaryColor(Objects.requireNonNull(getActivity()))))
                .start(callback);
        return cab;
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        switch (id) {
            case R.id.action_equalizer:
                NavigationUtil.openEqualizer(Objects.requireNonNull(getActivity()));
                return true;
            case R.id.action_sleep_timer:
                new SleepTimerDialog().show(Objects.requireNonNull(getFragmentManager()), "SET_SLEEP_TIMER");
                return true;
